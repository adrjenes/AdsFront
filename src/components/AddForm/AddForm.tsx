...
export const AddForm = () => {
    ...
    if (id) { return <h2>Twoje ogłoszenie "{form.name}" zostało poprawnie dodane do serwisu pod ID: {id}.</h2>}
    return (
        <form action="" className="add-form" onSubmit={saveAd}>
            ...
            <p><label>
                Cena: <br/>
                <input type="number" name="price" maxLength={99} value={form.price} onChange={e => updateForm('price', Number(e.target.value))}/>
            </label></p>
            <p><label>
                Adres URL: <br/>
            ...
            <p><label>
                ...
                <input type="text" name="address" required value={form.address} onChange={e => updateForm('address', e.target.value)}/>
            </label></p>
            <Btn text="Zapisz"/>
    </form>
    )
